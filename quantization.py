...
def quantizer(v,x,b,T):
    ...
    for _ in range(T):
        ...
        # Qloss = (B@v[:,None] - x).abs().mean()
        # print(f'QuantLoss:{Qloss}')
    return v,B
...
class Quantizer(torch.autograd.Function):
    ...
    def forward(ctx, x, qtFlag):
        ...
        if 'none' in qtFlag:
            ...
            # Quantizer.v = v if Quantizer.v is None else (1-alpha)*Quantizer.v + alpha*v
            # Quantizer.b = b if Quantizer.b is None else (1-alpha)*Quantizer.b + alpha*b
            Quantizer.v = v #if Quantizer.v is None else (1-alpha)*Quantizer.v + alpha*v
            Quantizer.b = b
            ...
            b = Quantizer.b
            # b = x.mean()
            # v = getUnifV(x, Quantizer.n_bit).cuda()
            # v, B = quantizer(v, x, b, 5)
...
# class ApproximateGrad(torch.autograd.Function):
#     @staticmethod
#     def forward(ctx, x, x_bar):
#         return x_bar
#
...
class SoftmaxQuantizer(torch.autograd.Function):
    def __init__(self, n_bit=6,a=8,v_sup=1):
