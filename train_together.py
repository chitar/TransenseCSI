...
parser.add_argument('--load_model', type=str, default=r'',
                    help='pre load model path')
...
parser.add_argument('--train_portion', type=float,default=0.3,
                    help='the training data portion over data set')
parser.add_argument('--lr', type=float,default=1e-4,
                    help='learning rate')
parser.add_argument('--T', type=int,default=10,
                    help='times of communication')
parser.add_argument('--loss', type=str, default='NMSEr',
                    help='loss function: NMSEr or Corr')
...
parser.add_argument('--trSNR', type=int,default=30,
                    help='noise level of estimation error in testing, -1 means none')
parser.add_argument('--fixM',  action='store_true')
parser.add_argument('--quantize',  type=str,default='none',help='none,uniform,opt')
...
save_dir = f'NN-{args.NN}-trSNR-{args.trSNR}-T-{args.T}-quantize-{args.quantize}-Q_mod-{args.Q_mod}-seed-{args.seed}-{time_now}'
print(save_dir)
...
if __name__ == '__main__':
    ...
    train_dataset = CSIDataset(data_filepath,train='train',train_portion=args.train_portion,seed=1,SNR_dB=args.trSNR,noise=args.noise)
    valid_dataset = CSIDataset(data_filepath,train='valid',train_portion=args.train_portion,seed=1 )
    test_dataset = CSIDataset(data_filepath,train='test',train_portion=args.train_portion,seed=1 )
    ...
    valid_loader =  torch.utils.data.DataLoader(valid_dataset, batch_size=BATCH_SIZE, shuffle=False, pin_memory=True, num_workers=NUM_WORKERS)
    test_loader = torch.utils.data.DataLoader(test_dataset, batch_size=BATCH_SIZE, shuffle=False, pin_memory=True, num_workers=NUM_WORKERS)


    model = CSIModel(input_size=64, hidden_size=512, Na=32, T=args.T, Np=32,n_layer=1, fixM=args.fixM, seed=args.seed, model=args.NN,datapath=data_filepath,Q_mod=args.Q_mod).cuda()
    ...
    for epoch in range(EPOCHS):
        ...
        for step, data in enumerate(train_loader):
            ...
            H = data['H'].cuda()
            H_hats, ps, cqis, Qs,ms = model(H.reshape(H.shape[0],-1))
            H_hat = H_hats.reshape(H_hats.shape[0],-1,32,2)

            # # check singular value of Q
            # svdValues = torch.linalg.svd(Qs)[1][:,:,0:8]
            # ratio = svdValues/svdValues.sum(dim=-1,keepdims=True)
            H_hat = torch.view_as_complex(H_hat.contiguous())
            H = torch.view_as_complex(H.contiguous())

            nmser_loss = NMSEr(H=H[None,:,:],H_hat=H_hat)
            corr_loss = 1-CORR(H=H[None,:,:],H_hat=H_hat)

            rloss = nmser_loss.mean() if args.loss == 'NMSEr' else corr_loss[-1] if args.loss == 'Corr' else None
        ...
        if epoch % TEST_FRE == TEST_FRE - 1:
            ...
            with torch.no_grad():
                ...
                val_loss = val_nmser_loss[-1] if args.loss == 'NMSEr' else val_corr_loss[-1] if args.loss == 'Corr' else None
                print('Val: meanLoss {:.6f}  nmser_loss {} '.format(
                    val_loss, 10 * torch.log10(val_nmser_loss),))
                ...
                if val_loss <= min_loss :
                    ...
                    # save data
                    torch.save({'H': total_H, 'H_hat': total_H_hat, 'Hr': total_Hr, 'P': total_P, 'CQI': total_CQI, 'Qs': total_Q},
                               os.path.join(save_dir, 'out_data.pth'))
    ...
    with torch.no_grad():
        ...
        val_loss = val_nmser_loss[-1] if args.loss == 'NMSEr' else val_corr_loss[
            -1] if args.loss == 'Corr' else None
        ...
        # save data
        torch.save(
            {'H': total_H, 'H_hat': total_H_hat, 'Hr': total_Hr, 'P': total_P, 'CQI': total_CQI,
             'Qs': total_Q},
            os.path.join(save_dir, 'test_data.pth'))
