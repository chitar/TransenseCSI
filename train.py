...
data_filepath = r'./data/data_DCSnn/data_DCSnn.mat'
#
...
if __name__ == '__main__':
    ...
    train_dataset = CSIDataset(data_filepath,train='train',train_portion=args.train_portion,seed=args.seed,SNR_dB=args.trSNR,noise=args.noise,nB=nB)
    valid_dataset = CSIDataset(data_filepath,train='valid',train_portion=args.train_portion,seed=args.seed ,nB=nB)
    test_dataset = CSIDataset(data_filepath,train='test',train_portion=args.train_portion,seed=args.seed ,nB=nB)
    ...
    model = CSIModel(input_size=64, hidden_size=512, Na=32, T=args.T, Np=32,n_layer=2, fixM=args.fixM, seed=args.seed, model=args.NN,datapath=data_filepath,Q_mod=args.Q_mod,nB=nB).cuda()
    ...
    for epoch in range(EPOCHS):
        ...
        for step, data in enumerate(train_loader):
            ...
            if step % PRINT_FRE == 0:
                print( f'Epoch: [{epoch}][{step}/{len(train_loader)}] Loss: {avg_loss:.6f} ')
