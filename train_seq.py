...
parser.add_argument('--NN', type=str, default=r'LSTM',
                    help='Atten,LSTM')
...
parser.add_argument('--loss', type=str, default='individual',
                    help='loss function: weighted,individual')
...
load_dir =  f'NN-{args.NN}-trSNR-{args.trSNR}-T-{args.T-1}-quantize-{args.quantize}-Q_mod-{args.Q_mod}-seed-{args.seed}'
args.load_model = os.path.join(args.exp_name,load_dir,'model_best.pth')  if args.T > 1 else ''
