...
class CSICell(nn.Module):
    def __init__(self,hidden_size,Na = 32, Np=8,n_layer = 2,model='Atten',datapath='',qtFlag='None',Q_mod='eye',nB=16):
        super(CSICell, self).__init__()
        self.qtFlag = qtFlag
        self.Np = Np
        self.Na = Na
        # self.V = torch.load('codebook_8d.pth')['codebook'].type(torch.cfloat).cuda()
        ...
        # self.V = torch.from_numpy(np.array(io.loadmat(r'../../QuaDriGa_data/data_DCSnn_QuaDriGa20000s.mat')['M_CB_T1_s'])).type(torch.cfloat).cuda()
        self.hidden_size = hidden_size
        self.cell =LSTMCell(Na * 2, hidden_size,num_layers=n_layer) if model=='LSTM' else AttenCell(Na*2,hidden_size,num_layers=n_layer)
        self.Q_fc = LSTM_Q_FC(hidden_size,nB) if model=='LSTM' else Atten_Q_FC(hidden_size,nB)
    ...
    def forward(self,H,Q,dec_hcs_pre,R):
        ...
        if Q is None:
            if self.q_mod=='learned':
                # ini = torch.zeros(H.shape[0],self.hidden_size).cuda()
                # Q = self.Q_fc(ini)
                # Q = Q.reshape(Q.shape[0], H.shape[2], self.Np, 2)
        ...
        # # --BS's operations
        # # self.V[:, m].t()
        # h_bar = cqi[:, None] * v / v.norm(dim=1)[:, None] ** 2
        # # ----convert to real number
        # h_bar = torch.view_as_real(h_bar).reshape(h_bar.shape[0], -1)
        ...
        Q_next = torch.view_as_complex(torch.zeros(nBatch, 32, 32, 2, requires_grad=True).cuda())
        Q_pre = self.Q_fc(dec_hcs['Q_feature'])
        Q_pre = torch.view_as_complex(Q_pre.reshape(nBatch, 32, -1, 2))
        R_H = R.transpose(-1,-2).conj()
        Proj = R@(R_H@R).inverse()@R_H
        Q_pre = Proj@Q_pre
        Q_next[:, :, 0:Q_pre.shape[-1]] = Q_pre
...
class CSIModel(nn.Module):
    def __init__(self,input_size,hidden_size,T,Na,Np,fixM,seed,n_layer=1,model = 'Atten',datapath='',qtFlag='none',Q_mod='eye',nB=16):
        super(CSIModel, self).__init__()
        np.random.seed(seed)
        self.fixM = fixM
        self.Q = torch.Tensor(np.random.randn(T, Na, Na, 2)).cuda()
        self.T = T
        self.cell = CSICell(hidden_size,Na,Np,n_layer,model,datapath,qtFlag,Q_mod=Q_mod,nB=nB)
        # self.fc = nn.Linear(hidden_size,input_size)

        # self.cells = [ CSILSTMCell(input_size,hidden_size,out_size,decoder_layers,code_size,Np) for i in range(T)]
    ...
    def forward(self,H,R):
        ...
        for i in range(self.T):
            # Q = torch.randn(64,8,2)[None,:,:,:].repeat(64,1,1,1)
            # Q,dec_hcs, p, cqi = self.cells[i](H,Q,dec_hcs)
            Q_pre,Q, dec_hcs, p, cqi,csi_next,m = self.cell(H, Q, dec_hcs,R)

            if self.fixM:
                # set the predicted Q as a fixed matrix
