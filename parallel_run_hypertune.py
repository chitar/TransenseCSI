...
def loopParam(knownParams, allParams, paramInd,cmds):
    if paramInd == len(allParams):
        ...
        cmds.append(f"python train_hypertune.py --NN {NN} --exp_name  {exp_name} --trSNR {trSNR} --epoch {epoch} --T {T} --Q_mod {Q_mod} --seed {paraDict['seed']} --quantize {quantize} --n_layer {paraDict['n_layer']} --n_dim {paraDict['n_dim']} --n_head {paraDict['n_head']}")
