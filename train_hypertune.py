...
parser.add_argument('--NN', type=str, default=r'Atten',
                    help='Atten,LSTM')
parser.add_argument('--exp_name', type=str, default='hyper_tuning',
...
# data_filepath = r'../data/QuaDriGa_data/data_DCSnn_QuaDriGa.mat'
# data_filepath = r'../data/QuaDriGa_data/data_DCSnn_QuaDriGa20000s'
#
...
save_dir = f'NN-{args.NN}-trSNR-{args.trSNR}-T-{args.T}-n_layer-{args.n_layer}-n_dim-{args.n_dim}-n_head-{args.n_head}-quantize-{args.quantize}-Q_mod-{args.Q_mod}-seed-{args.seed}-{time_now}'
print(save_dir)
...
if __name__ == '__main__':
    ...
    train_dataset = CSIDataset(data_filepath,train='train',train_portion=args.train_portion,seed=1,SNR_dB=args.trSNR,noise=args.noise,nB=nB)
    ...
    model = CSIModel(input_size=64, hidden_size=args.n_dim, Na=32, T=args.T, Np=32,n_layer=args.n_layer, fixM=args.fixM, seed=args.seed, model=args.NN,datapath=data_filepath,Q_mod=args.Q_mod,nB=nB,n_head=args.n_head).cuda()
    ...
    for epoch in range(EPOCHS):
        ...
        for step, data in enumerate(train_loader):
            ...
            # H_hat = H_hats.reshape(H_hats.shape[0],-1,nB,2)
            ...
            # H_hat = R[None,None,:,:]@H_hat[:,:,:,None]
            # H_hat = H_hats.squeeze(dim=-1)
